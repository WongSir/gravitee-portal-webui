...
import { PortalService, Ticket, TicketsResponse } from '@gravitee/ng-portal-webclient';
...
describe('GvTicketsHistoryComponent', () => {
  ...
  beforeEach(async(() => {
    ...
    spyOn(ConfigurationService.prototype, 'get')
      .withArgs('pagination.size.values').and.returnValue(PAGE_SIZES)
      .withArgs('pagination.size.default').and.returnValue(PAGE_SIZE_DEFAULT);
  ...
  it('should get ticket data without api path param and without selected ticket', async () => {
    ...
    spyOn(portalService, 'search').and.returnValue(of(response));
  ...
  it('should get field order for desc order query', () => {
    route.snapshot.queryParams.field = 'subject';
    route.snapshot.queryParams.order = 'DESC';

    expect(component.getOrder()).toEqual('-subject');
  });

  it('should get field order for asc order query', () => {
    route.snapshot.queryParams.field = 'subject';
    route.snapshot.queryParams.order = 'ASC';
  ...
  it('should navigate with new query params when selecting page size', fakeAsync(() => {
    ...
    expect(spy).toHaveBeenCalledWith([], {
      queryParams: { size: 5, ticket: null },
