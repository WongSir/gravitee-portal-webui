...
import { Log, PortalService, SearchRequestParams, Ticket } from '@gravitee/ng-portal-webclient';
...
export class TicketsHistoryComponent implements OnInit, OnDestroy {
  ...
  async refresh(queryParams) {
    ...
    const response = await this.portalService.search(this.getRequestParameters(queryParams, this.api)).toPromise();
  ...
  private getRequestParameters(queryParams, api): SearchRequestParams {
    const field = queryParams[SearchQueryParam.FIELD] || 'created_at';
    return {
